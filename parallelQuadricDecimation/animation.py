...
def play_animation(mesh_sequence, framerate=5, radius=0.005,
                   landmarks=None, mappings=None, radius_landmarks=0.01):
    ...
    def handle_timer(event):
        ...
        if landmarks is None:
            ...
            plotter.pop().pop().pop().pop().add([
                ...
                vedo.Mesh(mesh_sequence[i], c='k'),
                vedo.Spheres(mesh_sequence[i].points, r=radius, c='r'),
                vedo.Spheres(mesh_sequence[i].points[mappings[i][landmarks]], r=radius_landmarks, c='b')
            ]).render()
    ...
    if landmarks is None:
        ...
        plotter.show(
            ...
            vedo.Mesh(mesh_sequence[0], c='k'),
            vedo.Spheres(mesh_sequence[0].points, r=radius, c='r'),
            vedo.Spheres(mesh_sequence[0].points[mappings[0][landmarks]], r=radius_landmarks, c='b')
            ]
        )
...
for stop_at in [
        ...
        7500, 8000, 8500,  9000, 9500, 9750,
        10000, 10250, 10500, 11000, 11125, 11250,
        11500, 11750, 12000, 12100, 12200, 12300,
        12400]:

        # decimated_mesh, indice_mapping = progressive_decimation(mesh, stop_at, d, use_numba=True)
...
play_animation(successive_decimations, framerate=8, radius=0.003, radius_landmarks=0.01, landmarks=landmarks, mappings=indices_mappings)